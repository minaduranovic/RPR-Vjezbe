...
public class GeografijaDAO {
    ...
    private GeografijaDAO() {
        ...
        try {
            glavniGradUpit = conn.prepareStatement("SELECT grad.grad_id, grad.naziv, grad.broj_stanovnika, grad.drzava FROM grad, drzava WHERE grad.drzava=drzava.drzava_id AND drzava.naziv=?;");
        } catch (SQLException e) {
            regenerisiBazu();
            try {
                glavniGradUpit = conn.prepareStatement("SELECT grad.grad_id, grad.naziv, grad.broj_stanovnika, grad.drzava FROM grad, drzava WHERE grad.drzava=drzava.drzava_id AND drzava.naziv=?;");
